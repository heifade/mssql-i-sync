...
export let des = {
  ...
  alg: "des-ede3-cbc",
  encryptStr: function(text: string, key: string) {
    return this.encrypt(Buffer.from(text, "utf8"), key).toString("base64");
  },
  ...
  decryptStr: function(text: string, key: string) {
    const buffer = Buffer.from(text, "base64");
    return this.decrypt(buffer, key).toString();
  },
